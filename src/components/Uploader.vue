      ...
      <b-modal ref="csvModal" size="lg" title="CSV Mapping maken" hide-footer>
        ...
        <b-form inline>
          ...
          >
            <b-form-select
              :options="csv.headers"
              v-model="csv.selected[field.name]"
    ...
    return {
      ...
      dropzoneOptions: {
        ...
        dictMaxFilesExceeded:
          "Maximaal aantal bestanden van {{ maxFiles }} bereikt."
      }
    };
  },
  methods: {
    ...
    readFile(file) {
      ...
      reader.onloadend = () => {
        ...
        if (this.files.length === this.queued) {
          this.upload();
        }
      };
    ...
    upload: function() {
      ...
      this.$api
        .post(this.$config.api.uri + "/files", formData)
        .then(() => {
      ...
      this.csv.mapped = reduce(
        ...
        (acc, arr) => {
          ...
          fields.forEach(field => {
            const name =
              testHeaders.indexOf(field.name) >= 0
                ? field.name + "@" + field.value
                : field.name;
            row[name] =
              field.columnIndex >= 0 && arr[field.columnIndex]
                ? arr[field.columnIndex]
                : "";
          });
